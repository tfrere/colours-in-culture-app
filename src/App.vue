...
export default {
  ...
  data() {
    return {
      ...
      lineWidth: 82,
      offset: 20,
      color: this.$cookie.get("color")
        ? "/color/" + this.$cookie.get("color")
        : "/color/blue",
      culture: this.$cookie.get("culture")
        ? "/culture/" + this.$cookie.get("culture")
        : "/culture/westernAmerican",
      concept: this.$cookie.get("concept")
        ? "/concept/" + this.$cookie.get("concept")
        : "/concept/mourning",
    };
  },
    ...
    activation() {
      ...
      if (this.$route.params.color) {
        ...
        this.updateLineWidth(
          this.$refs.colorButton.$el.clientWidth,
          this.$refs.colorButton.$el.offsetLeft
        );
        this.isNavActive = true;
      } else if (this.$route.params.culture) {
        this.mode = "culture";
        this.updateLineWidth(
          this.$refs.cultureButton.$el.clientWidth,
          this.$refs.cultureButton.$el.offsetLeft
        );
        this.isNavActive = true;
      } else if (this.$route.params.concept) {
        this.mode = "concept";
        this.updateLineWidth(
          this.$refs.conceptButton.$el.clientWidth,
          this.$refs.conceptButton.$el.offsetLeft
        );
        this.isNavActive = true;
      }
      setTimeout(() => {
  ...
  watch: {
    $route(to) {
      ...
      this.color = this.$cookie.get("color")
        ? "/color/" + this.$cookie.get("color")
        : "/color/blue";
      this.culture = this.$cookie.get("culture")
        ? "/culture/" + this.$cookie.get("culture")
        : "/culture/westernAmerican";
      this.concept = this.$cookie.get("concept")
        ? "/concept/" + this.$cookie.get("concept")
        : "/concept/mourning";

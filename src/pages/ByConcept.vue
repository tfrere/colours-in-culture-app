  ...
  <div class="page"  :class="{active: isActive, animate: hasChanged}">
    ...
    <masonry
      ...
      <div class="masonry-card" :class="tag.color" v-for="(tag, index) in response" :key="index">
        ...
        <div class="big-label">
          <span class="name" v-on:click="() => { goToCulture(tag.culture) }">{{ tag.label }}</span>
        </div>
      </div>
    </masonry>
...
let findTagByName = function(value) {
  for (var i = 0; i < associatedTags.length; i++) {
    if (associatedTags[i].value == value) return associatedTags[i];
  }
};
...
export default {
  ...
  data() {
    return {
      ...
      dominant: findTagByName(this.$route.params.concept).pie.dominant[0]
        ? findTagByName(this.$route.params.concept).pie.dominant
        : "white",
      response: {},
      hasChanged: false,
      title: "Concept",
  ...
  methods: {
    nameWithDataLength({ name, dataLength }) {
      // , datalength
